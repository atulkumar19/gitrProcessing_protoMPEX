...
hold on
if (exist('x1') == 0)
    fid = fopen(strcat(pwd,'/Angled02_walls_0.cfg'));
...
patch(transpose(X(surface,:)),transpose(Y(surface,:)),transpose(Z(surface,:)),a(surface),'FaceAlpha',.3)
...
title({'Al flux for the', 'High Density Magnetized Case'})
...
AverageRadialDensity1=(mean(RadialDensity1));
figure;
plot(gridR,AverageRadialDensity1)
xlim([0 0.06])

title({'Al density for the', 'High Density Magnetized Case'})
xlabel('Plasma Radius (m)')
ylabel('Al ion flux [Particles/m^2/s]')
set(gcf,'color','w')
set(gca,'fontsize',13)
